...
export const generateCityGoal = async (stats: CityStats, grid: Grid): Promise<AIGoal | null> => {
  ...
  try {
    ...
    if (error.status === 429 || error.code === 429 || (error.message && error.message.includes('429'))) {
        console.warn("Gemini API Quota Exceeded (Goal Gen). Retrying later.");
        return null;
    }
...
export const generateNewsEvent = async (stats: CityStats, recentAction: string | null): Promise<NewsItem | null> => {
  ...
  try {
    ...
    if (error.status === 429 || error.code === 429 || (error.message && error.message.includes('429'))) {
        console.warn("Gemini API Quota Exceeded (News Gen). Skipping.");
        return null;
    }
...
// --- Hub Greeting ---
export const generateHubGreeting = async (): Promise<string> => {
    const prompt = "You are an enthusiastic game hub secretary character (like in Kairosoft games). Write a very short (max 10 words) welcome message for the player to select a simulation.";
    try {
        const response = await ai.models.generateContent({
            model: modelId,
            contents: prompt,
        ...
        return response.text?.trim() || "Welcome back, CEO! Select a simulation.";
    } catch (e) {
        // Silently fail to default
        return "Welcome back! Select a simulation.";
    }
}