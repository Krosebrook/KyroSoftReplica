...
const InfoTooltip = ({ buildingType }: { buildingType: BuildingType }) => {
    const config = BUILDINGS[buildingType];
    return (
        <Html center position={[0, 2, 0]} className="pointer-events-none whitespace-nowrap z-50">
            <div className="bg-slate-900/95 text-white p-3 rounded-lg border border-slate-500 shadow-2xl text-xs backdrop-blur-md flex flex-col gap-1 min-w-[120px] transition-all duration-200 ease-out transform translate-y-0 opacity-100">
                <div className="font-bold text-cyan-400 border-b border-slate-700 pb-1 text-sm tracking-wide">{config.name}</div>
                <div className="text-slate-300 italic text-[10px] leading-tight py-1">{config.description}</div>
                {(config.popGen > 0 || config.incomeGen > 0) && (
                    <div className="grid grid-cols-2 gap-x-2 text-[10px] font-mono mt-1 bg-slate-800/50 p-1.5 rounded border border-slate-700/50">
                        {config.popGen > 0 && <><span className="text-blue-300 font-semibold">Pop:</span> <span className="text-right">+{config.popGen}</span></>}
                        {config.incomeGen > 0 && <><span className="text-green-300 font-semibold">Inc:</span> <span className="text-right">${config.incomeGen}</span></>}
                    </div>
                )}
                {/* Arrow */}
                <div className="absolute top-full left-1/2 -translate-x-1/2 -mt-1 w-2 h-2 bg-slate-900 border-r border-b border-slate-500 transform rotate-45"></div>
            </div>
        </Html>
    );
};

// --- 2. Advanced Procedural Buildings ---
...
const SmokeStack = ({ position, scale = [0.2, 1, 0.2] }: { position: [number, number, number], scale?: [number, number, number] }) => {
  ...
  return (
    <group position={position}>
      <mesh geometry={cylinderGeo} castShadow receiveShadow position={[0, 0.5, 0]} scale={scale}>
        ...
        switch (type) {
          case BuildingType.Residential:
             if (variant < 25) {
                ...
                return (
                    <>
                        <mesh {...commonProps} material={mainMat} geometry={boxGeo} position={[0, h/2, 0]} scale={[0.6, h, 0.6]} />
                        <mesh {...commonProps} material={roofMat} geometry={boxGeo} position={[0, h, 0]} scale={[0.5, 0.1, 0.5]} />
                        {Array.from({length: Math.floor(h*3)}).map((_, i) => (
                             <WindowBlock key={i} position={[0, 0.2 + i*0.3, 0.31]} scale={[0.4, 0.15, 0.05]} />
                        ))}
                    </>
                )
             } else if (variant < 50) {
              ...
              return (
                <>
                  ...
                  <mesh {...commonProps} material={roofMat} geometry={boxGeo} position={[0.25, 0.55, 0.1]} scale={[0.3, 0.05, 0.5]} />
                </>
              );
            } else {
              // Townhouse / Duplex
              return (
                <>
                  <mesh {...commonProps} material={mainMat} geometry={boxGeo} position={[0.2, 0.4, 0]} scale={[0.4, 0.8, 0.6]} />
                  <mesh {...commonProps} material={mainMat} geometry={boxGeo} position={[-0.2, 0.4, 0]} scale={[0.4, 0.8, 0.6]} />
                  <mesh {...commonProps} material={roofMat} geometry={boxGeo} position={[0, 0.85, 0]} scale={[0.9, 0.1, 0.7]} />
                  <WindowBlock position={[0.2, 0.5, 0.31]} scale={[0.2, 0.2, 0.05]} />
                  <WindowBlock position={[-0.2, 0.5, 0.31]} scale={[0.2, 0.2, 0.05]} />
                </>
              );
            }
          ...
          case BuildingType.Commercial:
            if (variant < 30) {
              // Office Tower
              const height = 1.8 + hash;
              return (
                <>
                  <mesh {...commonProps} material={new THREE.MeshStandardMaterial({color: '#93c5fd', metalness: 0.8, roughness: 0.1})} geometry={boxGeo} position={[0, height/2, 0]} scale={[0.6, height, 0.6]} />
                  <mesh {...commonProps} material={accentMat} geometry={boxGeo} position={[0, 0.2, 0]} scale={[0.7, 0.4, 0.7]} />
                  <mesh {...commonProps} material={accentMat} geometry={boxGeo} position={[0, height, 0]} scale={[0.5, 0.1, 0.5]} />
                </>
              );
            } else if (variant < 60) {
              // High-rise
              ...
              return (
                <>
                  ...
                  <mesh {...commonProps} material={roofMat} geometry={boxGeo} position={[0, 0.65, 0]} scale={[0.8, 0.1, 0.8]} />
                  <WindowBlock position={[0, 0.3, 0.46]} scale={[0.8, 0.3, 0.05]} />
                  <mesh {...commonProps} material={accentMat} geometry={boxGeo} position={[0.25, 0.8, 0.25]} scale={[0.3, 0.2, 0.3]} />
                  <mesh {...commonProps} material={new THREE.MeshStandardMaterial({color: '#ef4444'})} geometry={boxGeo} position={[-0.3, 0.8, -0.3]} scale={[0.1, 0.3, 0.1]} />
                </>
              );
            }
            ...
            if (variant < 33) {
                ...
                return (
                    <>
                        ...
                        <mesh {...commonProps} material={accentMat} geometry={cylinderGeo} position={[-0.2, 0.5, -0.2]} scale={[0.15, 0.8, 0.15]} />
                        <mesh {...commonProps} material={accentMat} geometry={cylinderGeo} position={[0.2, 0.6, 0.2]} scale={[0.15, 1, 0.15]} />
                        <mesh {...commonProps} material={new THREE.MeshStandardMaterial({color: '#64748b'})} geometry={boxGeo} position={[0, 0.8, 0]} scale={[0.6, 0.05, 0.6]} />
                    </>
                )
            } else if (variant < 66) {
              // Factory
...
// --- 3. Dynamic Systems (Traffic, Citizens, Environment) ---
...
const GroundTile = React.memo(({ type, x, y, grid, onHover, onLeave, onClick, terrain, isVisible }: GroundTileProps) => {
  const [wx, _, wz] = gridToWorld(x, y);

  if (!isVisible) {
      return (
          <mesh position={[wx, 0, wz]}>
              <FogBlock />
          </mesh>
      );
  }
  ...
  if (terrain === 'island') {
      ...
      if (dist > 6) {
          color = '#1e293b'; // Very dark slate for unbuildable rock
          topY = -0.45; // Lower down
      }
  } else if (terrain === 'coast') {
      if (x < 4) {
          isRendered = false; // Don't render, show water
      } else if (x >= 4 && x <= 6) {
          color = '#fde047'; // Sand
          topY = -0.32;
      }
  ...
  if (isRendered) {
      if (type === BuildingType.None) {
        if (color === '#10b981') { // Only apply grass noise if not overridden by terrain
            const noise = getHash(x, y);
            color = noise > 0.7 ? '#059669' : noise > 0.3 ? '#10b981' : '#34d399';
            topY = topY - noise * 0.1; // Slight height variation for grass
        }
...
// --- Camera Rig for Smooth Animation ---
const CameraRig = ({ focusPoint }: { focusPoint: THREE.Vector3 | null }) => {
    const { controls } = useThree();
    const vec = useMemo(() => new THREE.Vector3(), []);

    useFrame((state, delta) => {
        if (focusPoint && controls) {
            const c = controls as any;
            if (c.target) {
                // Smoothly interpolate the controls target to the focus point
                c.target.lerp(focusPoint, delta * 3); 
                c.update();
            }
        }
    });
...
const IsoMap: React.FC<IsoMapProps> = ({ grid, onTileClick, hoveredTool, population, terrain }) => {
  ...
  const visibleSet = useMemo(() => {
    ...
    // Base Visibility (Center area)
    for(let y=0; y<GRID_SIZE; y++) {
        for(let x=0; x<GRID_SIZE; x++) {
             if (Math.sqrt((x-center)**2 + (y-center)**2) <= 5) {
                 visible.add(`${x},${y}`);
             }
        }
    }
    ...
    grid.flat().forEach(tile => {
        if (tile.buildingType !== BuildingType.None) {
             // Reveal radius 3 around buildings
  ...
  const handleClick = (x: number, y: number) => {
      onTileClick(x, y);
      const [wx, _, wz] = gridToWorld(x, y);
      setFocusPoint(new THREE.Vector3(wx, -0.5, wz));
  };

  // Validation Logic
  const isValidPlacement = (x: number, y: number, tool: BuildingType, terrain: TerrainType, tileData: TileData, isVisible: boolean) => {
      // 0. Visibility Check
      ...
      if (tool !== BuildingType.None) {
          // Building: Tile must be empty
          if (tileData.buildingType !== BuildingType.None) return false;
      } else {
          // Bulldoze: Tile must not be empty
          if (tileData.buildingType === BuildingType.None) return false;
      }
  ...
  let isValid = false;
  let previewPos = [0,0,0];

  if (hoveredTile) {
      const tileData = grid[hoveredTile.y][hoveredTile.x];
      const isVisible = visibleSet.has(`${hoveredTile.x},${hoveredTile.y}`);
      
      if (!isBulldoze) {
          showPreview = true;
          isValid = isValidPlacement(hoveredTile.x, hoveredTile.y, hoveredTool, terrain, tileData, isVisible);
      } else {
          isValid = tileData.buildingType !== BuildingType.None && isVisible;
      }
      previewPos = gridToWorld(hoveredTile.x, hoveredTile.y);
  }

  const previewColor = isValid ? BUILDINGS[hoveredTool].color : '#ef4444';
  const cursorColor = isValid ? (isBulldoze ? '#ef4444' : '#ffffff') : '#ef4444';
                    ...
                    <group position={[wx, 0, wz]} raycast={() => null}>
                        ...
                        <group>
                            ...
                            {isHovered && !showPreview && (
                                <InfoTooltip buildingType={tile.buildingType} />
                            )}
                        </group>
                        )}
                    </group>
