...
const ToolButton: React.FC<{
  ...
  return (
    <div className="group relative flex flex-col items-center">
        {/* Tooltip */}
        <div className="absolute bottom-full mb-3 hidden group-hover:flex flex-col bg-slate-800 text-white text-[10px] p-2 rounded-lg border border-slate-600 shadow-xl whitespace-nowrap z-50 pointer-events-none min-w-[120px]">
            <div className="font-bold text-xs text-cyan-300 mb-1 border-b border-slate-600 pb-1">{config.name}</div>
            <div className="text-slate-300 italic mb-2">{config.description}</div>
            
            {!isBulldoze && (
                <div className="grid grid-cols-2 gap-x-3 gap-y-1 font-mono">
                    <span className="text-red-300">Cost:</span>
                    <span className="text-right">${config.cost}</span>
                    
                    {config.incomeGen > 0 && (
                        <>
                            <span className="text-green-300">Income:</span>
                            <span className="text-right">+${config.incomeGen}/d</span>
                        </>
                    )}
                    {config.popGen > 0 && (
                        <>
                            <span className="text-blue-300">Pop:</span>
                            <span className="text-right">+{config.popGen}/d</span>
                        </>
                    )}
                </div>
            )}
            ...
            {/* Arrow */}
            <div className="absolute top-full left-1/2 -translate-x-1/2 -mt-px w-2 h-2 bg-slate-800 border-r border-b border-slate-600 transform rotate-45"></div>
        </div>
...
const UIOverlay: React.FC<UIOverlayProps> = ({
  ...
  return (
    <div className="absolute inset-0 pointer-events-none flex flex-col justify-between p-2 md:p-4 font-sans z-10">
      ...
      {/* Top Bar: Stats & Goal & Exit */}
      <div className="flex flex-col md:flex-row md:justify-between md:items-start pointer-events-auto gap-2 w-full max-w-full relative">
        
        {/* Mobile: Exit button absolute top right */}
        <button 
            onClick={onExit}
            className="md:hidden absolute top-0 right-0 bg-red-600/80 text-white p-2 rounded-lg border border-red-400/50 shadow-lg text-[10px] font-bold uppercase tracking-wider hover:bg-red-500"
        >
            Quit
        </button>

        {/* Stats */}
        <div className="bg-gray-900/90 text-white p-2 md:p-3 rounded-xl border border-gray-700 shadow-2xl backdrop-blur-md flex gap-3 md:gap-6 items-center justify-between md:justify-start w-full md:w-auto mr-12 md:mr-0">
        ...
        <div className="flex gap-2 items-start justify-end w-full md:w-auto">
            ...
            <button 
                onClick={onExit}
                className="hidden md:block bg-red-600/80 text-white py-2 px-4 rounded-xl border border-red-400/50 shadow-lg text-xs font-bold uppercase tracking-wider hover:bg-red-500 hover:scale-105 transition-all h-full"
            >
                Return to Hub
            </button>
      ...
      <div className="flex flex-col-reverse md:flex-row md:justify-between md:items-end pointer-events-auto mt-auto gap-2 w-full max-w-full">
        ...
        <div className="flex gap-1 md:gap-2 bg-gray-900/80 p-1 md:p-2 rounded-2xl border border-gray-600/50 backdrop-blur-xl shadow-2xl w-full md:w-auto overflow-x-auto no-scrollbar justify-start md:justify-start overflow-visible">
          <div className="flex gap-1 md:gap-2 min-w-max px-1 items-end">
            {tools.map((type) => (
              <ToolButton
                key={type}
                type={type}
          ...
          <div className="text-[8px] text-gray-500 uppercase writing-mode-vertical flex items-center justify-center font-bold tracking-widest border-l border-gray-700 pl-1 ml-1 select-none self-stretch">Build</div>
        </div>
